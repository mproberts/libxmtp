...
private const val ANVIL_TEST_PORT = "http://localhost:8545"

/** LOCAL environment pointed at localhost for the pure-JVM E2E suite. */
fun localApi(): JvmApi = JvmApi(env = XMTPEnvironment.LOCAL.withValue("localhost"))
...
class Fixtures(
    ...
    var alixClient: JvmClient =
        runBlocking { JvmClient.create(account = alixAccount, options = clientOptions) }

    var bo: PrivateKey = boAccount.getPrivateKey()
    var boClient: JvmClient =
        runBlocking { JvmClient.create(account = boAccount, options = clientOptions) }

    var caro: PrivateKey = caroAccount.getPrivateKey()
    var caroClient: JvmClient =
        runBlocking { JvmClient.create(account = caroAccount, options = clientOptions) }

    var davon: PrivateKey = davonAccount.getPrivateKey()
    var davonClient: JvmClient =
        runBlocking { JvmClient.create(account = davonAccount, options = clientOptions) }

    var eri: PrivateKey = eriAccount.getPrivateKey()
    var eriClient: JvmClient =
